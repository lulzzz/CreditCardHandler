...
extern "C"
{
	 ...
	 */
    DLLEXPORT const char*   RBA_SDK_GetVersion( void );
...
if (RBA_SDK_Reconnect() == RESULT_SUCCESS) {
	 ...
	 * 
	 *  If bReceive is true, the message response will be handled as blocking (synchronous),
	 *  and the function will wait for the device to respond before returning. If bReceive is false, the message 
	 ...
	 *  \param[in]  bReceive   <b>true</b> if the message response is handled as blocking;
	 ...
	 *  \sa RBA_SDK_SetProcessMessageSyncMode
     */
    DLLEXPORT int   RBA_SDK_SendCustomMessage( const char* RawData, int DataLength, bool bReceive );
    //! \cond INSTANCE_FUNCS
    DLLEXPORT int   RBA_SDK_SendCustomMessageFor( unsigned int instanceId, const char* RawData, int DataLength, bool bReceive );
    //! \endcond
